...
const ScatterPlotDualAudioFeatureComparison: React.FunctionComponent<IProps> = (props: IProps) => {
    ...
    const supportedTrackAudioFeaturesPerPlaylist: SpotifyApi.AudioFeaturesObject[][] = playlists.map(playlist => getSupportedTrackAudioFeaturesFromPlaylist(playlist, tracks));

    // Max and min points in the data
    const all_x_values: number[] = supportedTrackAudioFeaturesPerPlaylist.map(playlistAudioFeatures => playlistAudioFeatures.map(af => af[x_audio_feature.key] as number)).flat();
    const all_y_values: number[] = supportedTrackAudioFeaturesPerPlaylist.map(playlistAudioFeatures => playlistAudioFeatures.map(af => af[y_audio_feature.key] as number)).flat();
    ...
    return <Plot
        data={playlists.map((playlist: PlaylistObjectSimplifiedWithTrackIds, index: number) => ({
            x: supportedTrackAudioFeaturesPerPlaylist[index].map(af => af[x_audio_feature.key] as number),
            y: supportedTrackAudioFeaturesPerPlaylist[index].map(af => af[y_audio_feature.key] as number),
